      ...
      public function agentOccupation(Request $request) 
        {
          ...
          $user_and_profession = DB::table('occupation_user')
            ...
            ->select( 'users.name', 'users.email', 'occupation_user.price', 'occupation_user.user_id', 'users.hourly_rate', 'users.total_rating', 'users.member_since', 'users.avatar')
            ->get();

            \Log::alert($user_and_profession);
